...
namespace Presidents
{
    internal class Presidents
    {
        ...
        public void showPresidentByYear(int year)
        {
            var presidentList = getPresidentByYear(year);

            foreach (var president in presidentList)
            {
                Console.WriteLine($@"{president.Name}, {president.YearFrom}-{president.YearTo} parti: {president.Party}");
            }
        }

        public List<President> getPresidentByYear(int year)
        {
            List<President> presidentList = new List<President>();

            foreach(President president in _presidents)
            {
                int iterations = president.YearTo - president.YearFrom;

                for(int i = 0; i < iterations+1; i++)
                {
                    if((president.YearFrom + i) == year)
                    {
